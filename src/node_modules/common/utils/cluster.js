...
const isBoundaryOfSamePoint = boundary => {
  const boundObj = new LngLatBounds(boundary);
  return _.isEqual(boundObj.getNorthWest(), boundObj.getSouthEast());
};

const extendBounds = (boundary, radius = 100) => {
  const boundObj = new LngLatBounds(boundary);
  const nw = boundObj.getNorthWest();
  return _.flatten(nw.toBounds(radius).toArray());
};
