    ...
    _createClass(LayerWithOverlappedPointComponent, [{
      ...
      value: function _handleClick(properties, lngLat, event, meta, callback) {
        var _props = this.props,
            data = _props.data,
            onClickOverlappedPoints = _props.onClickOverlappedPoints;
        var map = this.context.map;

        var zoom = map.getMaxZoom() - 2;
        var points = (0, _utils.findPointsWithSameLocation)(data, lngLat, map, _ClusterOptions.ClusterOptions.NearestPointsRadius, zoom);
        if (points) {
